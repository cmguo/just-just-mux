...
#include "ppbox/mux/ts/elements.h"
...
namespace ppbox
{
    namespace mux
    {
        ...
        {
            // CABAC Decoding
            boost::int32_t           read_len;           //!< actual position in the codebuffer, CABAC only
            boost::int32_t           code_len;           //!< overall codebuffer length, CABAC only
            // CAVLC Decoding
            boost::int32_t           frame_bitoffset;    //!< actual position in the codebuffer, bit-oriented, CAVLC only
            boost::int32_t           bitstream_length;   //!< over codebuffer lnegth, byte oriented, CAVLC only
            // ErrorConcealment
            boost::uint8_t          *streamBuffer;      //!< actual codebuffer for read bytes
            boost::int32_t           ei_flag;            //!< error indication, 0: no error, else unspecified error
        };
        ...
        public:
            ...
            struct Nal_header
            {
                char nal_unit_type : 5;
                char nal_ref_idc : 2;
                char forbidden_zero_bit : 1;
            };
