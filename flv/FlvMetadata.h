...
namespace ppbox
{
    namespace mux
    {
        ...
        static inline void setTagSizeAndTimestamp(
            FlvTag & flv_tag, 
            boost::uint32_t size, 
            boost::uint32_t timestamp)
        {
            host_uint24_to_big_endian(size, flv_tag.DataSize);
            host_uint24_to_big_endian(timestamp, flv_tag.Timestamp);
            flv_tag.TimeExtand   = 0x00;
            flv_tag.Reserved[0]  = 0x00;
            flv_tag.Reserved[1]  = 0x00;
            flv_tag.Reserved[2]  = 0x00;
        }
